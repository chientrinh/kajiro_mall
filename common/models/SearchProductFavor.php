...
//        $rate = max($rate, $query->max('point_rate'));
    ...
    public function getPointForStaff()
    {
        ...
        if($ean13 = $this->item->barcode) {
            $q = StaffDiscount::find()->where(['ean13'=>$ean13]);
        ...
        if(!($this->item instanceof \common\components\cart\ComplexRemedyForm) && $subcategories = $this->item->getSubcategories()) {

            $subcatgory_array = ArrayHelper::getColumn($this->item->getSubcategories()->asArray()->all(),'subcategory_id');
            $q = StaffDiscount::find()->where(['in','subcategory_id',$subcatgory_array]);
