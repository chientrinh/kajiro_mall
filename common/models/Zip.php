...
class Zip extends \yii\db\ActiveRecord
{
    ...
    public function getMinDelivTimestamp($provider = self::PROVIDER_YAMATO, $time = null)
    {
        ...
        // 4月30日から正式に、青森県(pref_id:2)、秋田県(pref_id:5)の住所は北海道と同じ日数扱いとなるため、暫定措置。念の為伝票日付も見られるよう考慮
        if(($time && $time < strtotime('2021-04-30 00:00:00')) || time() < strtotime('2021-04-30 00:00:00')) {
            if($this->pref_id == 2 || $this->pref_id == 5) {
                $data = self::DELEVERY_15; // 青森・秋田は self::DELIVERY_15扱い
                $this->$provider = $data;
            }
        }
     ...
     */
    public function getDays($scenario = null, $purchase_date = null)
    {
        ...
        if(($purchase_date && $purchase_date < strtotime('2021-04-30 00:00:00')) || time() < strtotime('2021-04-30 00:00:00')) {
            if(isset($this->yamato_22) && ($this->pref_id == 2 || $this->pref_id == 5)) {
                return self::DAY_10_15; // 青森・秋田は self::DELIVERY_15扱い
            }
        }
