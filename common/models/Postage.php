...
class Postage extends \yii\base\Model
{
     ...
     * 冷蔵品（現状）＋冷凍品混在の場合、               
　   * ・合計2万円以上…         送料無料
　   * ・合計2万円以下…         冷蔵品が合計1万円以上…冷蔵品送料が無料。冷凍品送料は有料。
         *                    冷凍品が合計1万円以上…冷凍品送料が無料。冷蔵品送料は有料。
     */

    /**
     * 冷蔵品＋冷凍品
     */
    private $frozen_include_threshold = 20000;    

    /**
     * クール便（冷蔵 OR 冷凍便）の送料無料判定
     */
    private $cool_threshold = 10000;

    /**
     * 熱海・六本松（静岡県）からのヤマト便（冷凍便）料金
     * （[注意！！] あとからクール便一律200円を加算したりしない）
     */
    private $yamato_frozen_matrix = [
        1 =>  1100, //北海道
        2 =>  900, //青森県 // 北東北 900
        3 =>  900, //岩手県 // 北東北
        4 =>  770, //宮城県 // 南東北 770
    ...
    * クール便（冷蔵）の送料計算
    */
    public function getChilledValue()
    {
        if(in_array($this->payment_id, [Payment::PKEY_CASH,
                                        Payment::PKEY_CREDIT_CARD,
                                        Payment::PKEY_NO_CHARGE])
         ...
         // 合計1万円以上なら送料無料とする
         if($this->cool_threshold <= $this->taxable)
             return 0;
    ...
    public function getFrozenValue()
    {
        if(in_array($this->payment_id, [Payment::PKEY_CASH,
         ...
         $base = \yii\helpers\ArrayHelper::getValue($this->yamato_matrix,
                                                    $this->pref_id,
                                                    max($this->yamato_matrix));
