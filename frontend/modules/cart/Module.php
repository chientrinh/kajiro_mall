...
class Module extends \yii\base\Module
{
    ...
    public function addProduct($pid, $options)
    {
        ...
        if( $item ) {
            Yii::$app->getSession()->addFlash('success', sprintf("%s &yen;%s が %d 点 カートに>入りました", $item->name, number_format($item->price), $options['qty']). '<br/>' . \yii\helpers\Html::a("カートを見る",['/cart']));
        } else {
            // TODO: #443 特定商品では1個以上カートに追加出来ないようにする（後日、購入個数上限による制御に変更）
            if($pid == 2192) {
                Yii::$app->getSession()->addFlash('warning', 'この商品は一度のご注文で１個まで>しか購入いただけません');
                return false;
            }
     ...
     * @return bool
     */
    public function finish($cart_idx, $campaign_code = "")
    {
        $outOfStock = false;
        $msg = "注文を確定できませんでした。";
        ...
        try {
            ...
            foreach($items as $item)
            {
                ...
                if($purchase->agent_id) {                 
                       $direct_pointRate_1 = $item->getPointRate();
                       $agent_pointRate_1 = $purchase->getOfferRateByCustomer($item, $agent)['point'];
                       $agent_addPoint_1 = (floor($item->price * $agent_pointRate_1 / 100) - $item->getPointAmount());
                       $agent_addPoint_1 = $agent_addPoint_1 >= 0 ? $agent_addPoint_1 : 0;
                       $agent_point_1 += $agent_addPoint_1;
  

                       $product = CampaignDetail::find()
                       ->andWhere(['campaign_id' => $purchase->campaign_id, 
                       'ean13' => $item->model->barcode]);
                       $rate = $product->max('point_rate');

                     if ($rate) {
                         $agent_point += $item->qty * $this->getPointRateItem($item,$rate);
                      } else {
                         $agent_point += $item->qty * $agent_addPoint_1;
                      } 
...
//var_dump($agent_point,$agent_point_1); die;
   ...
   public function getPointRateItem($item,$rate){
            $point_amount = floor(($item->price - $item->getDiscountAmount()) * ($rate - $item->getPointRate()) / 100);
//   var_dump($point_amount,$campaign_id,$item->price,$item->getDiscountAmount(), $rate, $item->getPointRate()); die;           
if($point_amount >= 0){
                return $point_amount;
            }
...
*/
    ...
    public function updateCampaign($cart_idx, $params)
    {
        ...
        if(! isset($params['PurchaseForm']) || ($params['PurchaseForm'] < 0) || ! Yii::$app->user->identity || isset($params['PurchaseForm']['direct_code']))
